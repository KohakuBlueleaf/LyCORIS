...
def patch_getitem(self, index):
    ...
    for image_key in bucket[image_index : image_index + bucket_batch_size]:
        ...
        if image_info.latents is not None:  # cache_latents=Trueの場合
            ...
            crop_left_top = (
                image_info.latents_crop_left_top
            )  # calc values later if flipped
            if not flipped:
            ...
            latents, original_size, crop_left_top = self.load_latents_from_npz(
                image_info, flipped
            )
        ...
        if flipped:
            crop_left_top = (
                original_size[0] - crop_left_top[0] - target_size[0],
                crop_left_top[1],
            )
