...
class ConceptModule(nn.Module):
    ...
    def forward(self, x):
        ...
        return self.op(
            ...
            * (1 + (self.concept_mask - 1) * self.multiplier),
            bias,
            **self.extra_args,
        )
...
class ConceptNetwork(torch.nn.Module):
    ...
    def __init__(self, text_encoder, unet, multiplier=1.0, **kwargs) -> None:
        super().__init__()
        self.multiplier = multiplier
        ...
        print(
            f"create Concept for Text Encoder: {len(self.text_encoder_loras)} modules."
        )
