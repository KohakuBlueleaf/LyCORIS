...
class ConceptModule(nn.Module):
    def __init__(
        ...
        if org_module.__class__.__name__ == "Conv2d":
            ...
            self.op = F.conv2d
            self.extra_args = {"stride": stride, "padding": padding}
        else:
            self.op = F.linear
            self.extra_args = {}

        self.concept_mask = nn.Parameter(torch.ones_like(org_module.weight))
    ...
    def forward(self, x):
        ...
        if torch.max(self.concept_mask) - torch.min(self.concept_mask) < 0.1:
            return self.op(
                x,
                self.org_module[0].weight.data * self.concept_mask,
                bias,
                **self.extra_args,
            )
        else:
            ...
            self.concept_neuron = self.concept_mask * self.concept_neuron
            return self.op(
                x,
                self.org_module[0].weight.data
                * (1 + self.concept_mask * self.concept_neuron),
...
class ConceptNetwork(torch.nn.Module):
    ...
    def __init__(self, text_encoder, unet, multiplier=1.0, *args, **kwargs) -> None:
        super().__init__()
        self.multiplier = multiplier
            ...
            for name, module in root_module.named_modules():
                if module.__class__.__name__ in target_replace_modules:
                    for child_name, child_module in module.named_modules():
                        if child_name.rsplit(".", 1)[-1] not in [
                            "to_k",
                            "to_v",
                            "k_proj",
                            "v_proj",
        ...
        self.unet_loras = create_modules(
            ConceptNetwork.LORA_PREFIX_UNET,
            unet,
            ConceptNetwork.UNET_TARGET_REPLACE_MODULE,
        )
