...
class HadaModule(nn.Module):
    def __init__( self, orig_weight, shape, lora_dim):
        super().__init__()
    ...
    def forward(self, x):
        diff_weight = ((self.hada_w1_a@self.hada_w1_b)*(self.hada_w2_a@self.hada_w2_b))
        return self.orig_weight.reshape(diff_weight.shape) + diff_weight
...
class LohaModule(nn.Module):
    ...
    def __init__(
        ...
        orig_weight = self.org_module[0].weight.data
        # self.hada_w1_a = nn.Parameter(torch.empty(shape[0], lora_dim))
        # self.hada_w1_b = nn.Parameter(torch.empty(lora_dim, shape[1]))
        ...
        self.weight = HadaModule(orig_weight, shape, lora_dim)
